...
- `src/types.rs` — maps Athena/Glue types to DuckDB types, including native `DATE`/`TIMESTAMP` and `DECIMAL(width, scale)`; unmapped and unsupported complex types (array, map, struct) fall back to `Varchar`.
