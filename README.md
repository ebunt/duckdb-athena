...
- `predicate=` is checked at bind only for what Athena cannot catch: it must be a single expression, with no semicolons, SQL comments, or statement keywords. Column names are **not** pre-validated — Athena rejects an unknown column itself in about half a second having scanned 0 bytes (so at no cost), and its reason is surfaced verbatim: `COLUMN_NOT_FOUND: line 1:30: Column 'nosuchcol' cannot be resolved`. Pre-parsing the expression here meant maintaining a partial SQL grammar, which rejected valid queries such as `EXTRACT(year FROM ts) = 2024` whenever a keyword was missing from it
